...
export default async function handler(req, res) {
  ...
  let maxTokens = isDocGen ? 8000 : (isDiagram ? 8000 : 6000);
  ...
  if (isDocGen) {
    try {
      ...
      return res.status(200).json({ ...data, conversationId: convId, sources: webSources || undefined });
    } catch (err) {
  ...
  try {
    ...
    let continuations = 0;
    while (!clientDisconnected && continuations < 3 && (finishReason === 'length' || isUnclosedFence(fullReply))) {
      continuations++;
      try {
        const contMessages = [
          ...messages,
