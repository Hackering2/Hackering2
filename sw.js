...
// Réception d'une notification Push réelle (fonctionne même si l'app est complètement fermée)
self.addEventListener('push', (event) => {
  let data = { title: 'AgroAI', body: 'Nouvelle notification' };
  try { if (event.data) data = { ...data, ...event.data.json() }; } catch (e) {}

  event.waitUntil(
    self.registration.showNotification(data.title, {
      body: data.body,
      tag: data.tag || 'agroai-push',
      icon: 'https://raw.githubusercontent.com/Hackering2/Hackering2/principal/icon.png',
      badge: 'https://raw.githubusercontent.com/Hackering2/Hackering2/principal/icon.png',
      renotify: true,
      vibrate: [100, 50, 100]
    })
  );
